...
with col_ingreso:
    ...
    with col_f2:
        monto_total = st.number_input("Monto Total de Factura ($)", min_value=0.0, value=100000.0, step=1000.0)
    ...
    if tipo_gasto == "Servicio":
        ...
        if afecta_prod == "No":
            ...
            cc_seleccionados = []
            if b_corralitos: cc_seleccionados.append("201")
            if b_3dm: cc_seleccionados.append("301")
            if b_ambas: cc_seleccionados.append("101")
            if b_taller: cc_seleccionados.append("103")
            if b_admin: cc_seleccionados.append("1")
            if b_fin: cc_seleccionados.append("4")
            if b_imp: cc_seleccionados.append("5")
            ...
            if len(cc_seleccionados) > 0:
                ...
                st.caption("Puedes modificar los % manualmente. Si la suma no llega a 100%, el sobrante se asignará automáticamente al CC General.")
                ...
                if suma_pct > 100.0:
                    ...
                    if diferencia > 0.001:
                        ...
                        pct_ingresados[cc_ajuste] = pct_ingresados.get(cc_ajuste, 0.0) + diferencia
                        st.warning(f"ℹ️ La suma fue {suma_pct:.2f}%. Se asignaron **{diferencia:.2f}%** sobrantes a **{cc_ajuste} - {CC_DICT[cc_ajuste]}**.")
...
# 3. Servir la app incrustada en el mismo Colab mediante un iFrame de Google
